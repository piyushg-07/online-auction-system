...
def generate_conditioned_features(feasible=True):
    ...
    For feasible proposals:
        - proposal_cost is lower,
        - material_quality is high,
        - past_performance is high,
        - construction_complexity is low.
    For non-feasible proposals, force a violation of at least one condition.
    """
    if feasible:
        return {
            "proposal_cost": np.random.uniform(400000, 540000),  # below 550k
            "road_length": np.random.normal(loc=50, scale=20),
            "material_quality": np.random.randint(6, 11),         # greater than 5
            "contract_duration": np.random.randint(30, 365),
            "past_performance": np.random.uniform(0.6, 1),         # greater than 0.5
            "experience_years": np.random.randint(1, 40),
            "regional_infrastructure_quality": np.random.randint(1, 11),
            "traffic_density": np.random.uniform(0, 1),
            "environmental_impact_score": np.random.randint(1, 11),
            "regulatory_complexity": np.random.randint(1, 11),
            "construction_complexity": np.random.randint(1, 8),    # less than 8
            "budget_variance": np.random.uniform(0, 0.2),
            "maintenance_history": np.random.randint(1, 6)
        }
    else:
        # Force a violation: choose one condition to break.
        choices = {}
        ...
        if condition_break == "quality":
            choices["material_quality"] = np.random.randint(1, 6)  # 5 or less
        else:
            choices["material_quality"] = np.random.randint(1, 11)
        if condition_break == "performance":
            choices["past_performance"] = np.random.uniform(0, 0.4)  # Below threshold
        else:
            choices["past_performance"] = np.random.uniform(0, 1)
        if condition_break == "complexity":
            choices["construction_complexity"] = np.random.randint(8, 12)  # 8 or above
        else:
...
def generate_pdfs(n_pdfs, folder, feature_gen_func):
    """
    Generate n_pdfs PDF files using the feature generator function provided.
    folder: Directory to store the PDFs.
    feature_gen_func: A function that returns a dictionary of features.
    """
    for i in range(n_pdfs):
        data = feature_gen_func()
        filename = os.path.join(folder, f"proposal_{i+1}.pdf")
        create_pdf(data, filename)
    print(f"{n_pdfs} PDF files have been generated in the folder '{folder}'.")
...
def load_data_from_pdfs(folder):
    """Iterate through all PDFs in the given folder, extract and parse data, and return a DataFrame."""
    data_records = []
...
def train_model_from_pdf_data():
    """Load data from training PDFs, compute target, and train a Random Forest model."""
...
if __name__ == '__main__':
    ...
    # =========================================================================
    # 3. Load training data, train the model, and save it.
    model, train_df = train_model_from_pdf_data()
    save_model(model)
    
    # =========================================================================
    # 4. (Optional) Demonstrate prediction on a new PDF from random features.
    ...
    # Ensure the features DataFrame columns follow the expected order.
