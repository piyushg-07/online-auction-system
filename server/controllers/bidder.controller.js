...
const handleBidderLogin = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { bidderId: bidder._id, email: bidder.email },
      process.env.JWT_SECRET,
      { expiresIn: "14d" }
    );
...
const placeBid = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Bid placed successfully", product });
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
};
...
const updateBidderProfile = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Profile updated successfully", bidder: updatedBidder });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
...
const getBidderBids = async (req, res) => {
  ...
  try {
    ...
    const bidderBids = products.map((product) => ({
      ...
      bids: product.bids.filter(
        (bid) => bid.bidder._id.toString() === bidderId
      ),
    }));
